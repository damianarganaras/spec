...
**Input**: Optionally specify a change name after `/cleto-continue` (e.g., `/cleto-continue add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
4. **Act based on status**:
   ...
   **If all artifacts are complete (`isComplete: true`)**:
   ...
   - Suggest: "All artifacts created! You can now implement this change with `/cleto-apply` or archive it with `/cleto-archive`."
   - STOP
...
- Prompt: "Run `/cleto-continue` to create the next artifact"
